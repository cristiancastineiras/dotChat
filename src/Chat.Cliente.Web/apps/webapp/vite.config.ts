...
function crearAliasDesdePaquetes(): Record<string, string> {
	...
	try {
		for (const carpeta of fs.readdirSync(directorioPaquetes)) {
			...
			alias[packageJson.name] = fs.existsSync(rutaSrc) ? rutaSrc : raizPaquete
		}
	} catch (error) {
 ...
 */
function resolverVariablesEntorno(variables: Record<string, string>): Record<string, string> {
 ...
 */
function generarTiposEnv(variables: Record<string, string>, rutaSalida: string): void {
...
export default defineConfig(({ mode }: ConfigEnv): UserConfig => {
	...
	return {
		...
		server: {
			...
			// ======================================================================
			// PROXY HACIA EL SERVIDOR DE dotChat
			// ======================================================================
			// El servidor no publica cabeceras CORS a propósito: en un despliegue
			// real la app y la API salen del mismo nginx, así que no hay dos orígenes
			// que reconciliar. En desarrollo se reproduce esa misma situación desde
			// aquí, de modo que el navegador solo ve peticiones a su propio origen.
			//
			// `ws: true` en /hubs es imprescindible: SignalR negocia por HTTP y luego
			// sube la conexión a WebSocket; sin reenviar la actualización de
			// protocolo, la conexión caería al transporte de sondeo largo.
			proxy: variablesResueltas.VITE_SERVIDOR_URL
				? {
						"/api": {
							target: variablesResueltas.VITE_SERVIDOR_URL,
							changeOrigin: true,
							secure: false,
						},
						"/hubs": {
			...
			fs: {
				...
				allow: [path.resolve(__dirname, "../.."), path.resolve(__dirname, "../../paquetes")],
		...
		build: {
			...
			// Los grupos de vendor se declaran con `output.advancedChunks.groups`,
			// que es la API real de Rolldown y sustituye al `manualChunks` de Rollup.
			// Se castea a Record<string, unknown> porque los tipos `OutputOptions`
			// heredados aún no exponen `advancedChunks` ni `minify.compress`.
			rolldownOptions: {
				output: {
					...
					// Agrupación de dependencias por regex de ruta. Separa lo que cambia
					// con cada despliegue (el código de la app) de lo que no cambia casi
					// nunca (las librerías), para que un cambio de una línea no obligue al
					// navegador a volver a descargar antd entero.
					...(esProduccion
						? {
								advancedChunks: {
									// Cada módulo va al grupo cuyo patrón coincide con su propia
									// ruta, y no al de quien lo importa. Sin esto, el grupo de antd
									// se lleva por delante todo su árbol de dependencias —React
									// incluido— y los demás grupos quedan vacíos.
									...
									groups: [
										...
										{ name: "datos", test: /[\\/]node_modules[\\/](@tanstack|ky|zustand)[\\/]/ },
										{
											name: "tiempo-real",
